...
export class User {
  ...
  constructor({ name, email, password, preferences = {} }: any = {}) {
    this.name = name;
    this.email = email;
...
export default class UserController {
  ...
  static async logout(req: Request, res: Response) {
    try {
      ...
      const userObj: any = await User.decoded(userJwt);
      var { error } = userObj;
      if (error) {
  ...
  static async delete(req: Request, res: Response) {
    try {
      ...
      const userJwt = req.get('Authorization')?.slice('Bearer '.length) || '';
      const userClaim: any = await User.decoded(userJwt);
      var { error } = userClaim;
      if (error) {
        res.status(401).json({ error });
        return;
  ...
  static async save(req: Request, res: Response) {
    try {
      const userJwt = req.get('Authorization')?.slice('Bearer '.length) || '';
      const userFromHeader: any = await User.decoded(userJwt);
