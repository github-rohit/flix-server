...
export default class UsersDAO {
  static async injectDB(conn: any) {
    ...
    try {
      users = await conn.db(process.env.FLIX_NS).collection('users');
      sessions = await conn.db(process.env.FLIX_NS).collection('sessions');
    } catch (e) {
  ...
  */
   ...
   * @returns {DAOResponse} Returns either a "success" or an "error" Object
   */
  static async loginUser(email: string, jwt: string) {
    try {
      // TODO Ticket: User Management
   ...
   */
  static async getUserSession(email: string) {
    try {
      // TODO Ticket: User Management
   ...
   * @returns {DAOResponse}
   */
  static async updatePreferences(email: string, preferences: any) {
    try {
      /**
  ...
  static async checkAdmin(email: string) {
    try {
